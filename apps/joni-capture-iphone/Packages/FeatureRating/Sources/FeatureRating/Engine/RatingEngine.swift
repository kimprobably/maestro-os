...
public final class RatingEngine: @unchecked Sendable {
    ...
    public func record(_ action: RatingAction) -> Bool {
        ...
        AppLogger.debug(
            "Rating: recorded '\(action.id)' (\(action.sentiment.rawValue), w=\(action.weight)). Score: \(String(format: "%.2f", newScore)), actions: \(self.storage.totalActionCount())",
    ...
    public func userAcceptedPrompt() {
        ...
        AppLogger.info("Rating: user accepted prompt (hasRated=true). Prompts this year: \(self.storage.promptsThisYear())", category: AppLogger.ui)
    }
