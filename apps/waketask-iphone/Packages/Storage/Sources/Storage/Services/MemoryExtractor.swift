...
public final class KeywordMemoryExtractor: MemoryExtractor {
    ...
    public init() {
        ...
        reIAm = try! NSRegularExpression(pattern: #"\bi\s+am\b"#, options: .caseInsensitive)
        reIm = try! NSRegularExpression(pattern: #"\bi['']m\b"#, options: .caseInsensitive)
        reMyNameIs = try! NSRegularExpression(pattern: #"\bmy\s+name\s+is\b"#, options: .caseInsensitive)
        reLiveIn = try! NSRegularExpression(pattern: #"\bi\s+live\s+in\b"#, options: .caseInsensitive)
        reWorkAs = try! NSRegularExpression(pattern: #"\bi\s+work\s+as\b"#, options: .caseInsensitive)

        reILike = try! NSRegularExpression(pattern: #"\bi\s+like\b"#, options: .caseInsensitive)
        reILove = try! NSRegularExpression(pattern: #"\bi\s+love\b"#, options: .caseInsensitive)
        reIPrefer = try! NSRegularExpression(pattern: #"\bi\s+prefer\b"#, options: .caseInsensitive)
        reIHate = try! NSRegularExpression(pattern: #"\bi\s+hate\b"#, options: .caseInsensitive)
        reIDislike = try! NSRegularExpression(pattern: #"\bi\s+dislike\b"#, options: .caseInsensitive)
        reIEnjoy = try! NSRegularExpression(pattern: #"\bi\s+enjoy\b"#, options: .caseInsensitive)
        reDontLike = try! NSRegularExpression(pattern: #"\bi\s+don['']t\s+like\b"#, options: .caseInsensitive)

        reRememberThat = try! NSRegularExpression(pattern: #"\bremember\s+that\b"#, options: .caseInsensitive)
        reDontForget = try! NSRegularExpression(pattern: #"\bdon['']t\s+forget\b"#, options: .caseInsensitive)
        reKeepInMind = try! NSRegularExpression(pattern: #"\bkeep\s+in\s+mind\b"#, options: .caseInsensitive)
        reNoteThat = try! NSRegularExpression(pattern: #"\bnote\s+that\b"#, options: .caseInsensitive)

        reActually = try! NSRegularExpression(pattern: #"\bactually\b"#, options: .caseInsensitive)
        reIMeant = try! NSRegularExpression(pattern: #"\bi\s+meant\b"#, options: .caseInsensitive)
        reToClarify = try! NSRegularExpression(pattern: #"\bto\s+clarify\b"#, options: .caseInsensitive)
    }
    ...
    private func extractKeywords(from text: String) -> [String] {
        ...
        return Array(Set(words)).prefix(10).map(\.self)
    }
