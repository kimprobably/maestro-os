...
public final class FakeMessageRepository: MessageRepository, @unchecked Sendable {
    ...
    public func page(conversationID _: UUID, after: MessageCursor?, limit: Int) async throws -> (items: [MessageDTO], next: MessageCursor?) {
        if shouldThrow {
    ...
    public func deleteAll(in _: UUID) async throws {
        messages.removeAll()
    }
