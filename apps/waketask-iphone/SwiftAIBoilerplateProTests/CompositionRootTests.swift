...
final class CompositionRootTests: XCTestCase {
    ...
    func testSettingsViewModel_hasCorrectDependencies() {
        let viewModel = composition.makeSettingsViewModel()
    ...
    func testHomeViewModel_hasCorrectDependencies() {
        let viewModel = composition.makeHomeViewModel()
    ...
    func testCompositionRoot_withInvalidAuthConfig_throwsError() throws {
        // Test that invalid configuration is handled
        let invalidAuthConfig = try AuthConfig(
            supabaseURL: XCTUnwrap(URL(string: "invalid")),
            supabaseAnonKey: ""
        )
    ...
    func testFullChatFlow_withComposedDependencies() {
        let conversationID = UUID()
        let viewModel = composition.makeChatViewModel(conversationID: conversationID)
