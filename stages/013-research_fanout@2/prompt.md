...
- failure_signature: research_synthesis|deterministic|cli command exited with code <n>: or codex_api::endpoint::responses_websocket: failed to connect to websocket: http error: <n> unauthorized,url: wss://api.openai.com/v1/responses <n>-<n>-19t05:<n>:<n>.616026z error codex_api::endpoint::resp
...
Research Reddit conversations related to `Internal iPhone capture and operator console for Joni, Maestro LinkedIn agent. Implement the product slice from docs/operator/specs/active/joni-capture-iphone-app.md: lock-screen-adjacent/Shortcut/Action Button voice capture, safe driving capture UX, transcript/capture packet queue, Joni draft prep states, agent activity timeline, LinkedIn/Joni analytics dashboard, no LinkedIn mutation or auto-publishing.`. Look for:
